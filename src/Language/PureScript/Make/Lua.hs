...
modulePath (ModuleName ps) = foldr (</>) [] . map runProperName $ ps
  ...
  where
    ...
    codegen m _ exts = do
      ...
      let luaFile = outputDir </> filePath ++ ".lua"
          ...
          foreignFile = outputDir </> filePath ++ "_.lua"
          prefix =
